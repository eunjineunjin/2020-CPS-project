...
모두가 서버 구축과 연결의 문제를 해결하려고 노력하는 과정에서 배운 점은, Google Colaboratory가 로컬 PC가 아닌 클라우드 환경(Google Drive)에서 작업이 되기 때문에 같은 환경에서 서버 구축 등의 작업을 수행하지 않는 이상 연결이 어렵다는 점이다. 프로젝트 계획 전에 이 점에 대해 알지 못했기 때문에 서버 구축 및 어플 제작을 했음에도 불구하고 연결이 어려워 프로젝트를 완벽히 수행하지 못했다. 하지만, 프로젝트를 진행하면서 yolo 학습 중 가중치가 이상적(0.1 ~ 0.05)으로 낮춰지지 않아도 yolo가 영상 및 이미지에 대한 실시간 탐지를 위주로 하는 딥러닝 알고리즘이기 때문에 가중치가 1.1에 불과해도 객체에 대한 탐지는 어느정도 수행할 수 있다는 점을 배웠고, yolo 딥러닝 알고리즘의 구조를 전반적으로 알 수 있는 기회가 되었으며 커스텀 데이터 학습을 위해 파일들을 수정하면서 어떤 파일이 yolo 학습에 관여하는지를 알게 되었다. 또한 CPS 보안 실습 수업에서 배운 크롤링을 응용하여 구글 이미지 크롤러를 만드는 것 또한 수행할 수 있었다. 이 외에도 서버 구축 과정에서 기본적인 웹 구조와 동작 방식에 대해 배울 수 있었고 보통 아파치와 MySQL을 사용하여 웹서버를 만드는 방식 말고 파이썬을 기반으로 Flask를 사용하여 웹 서버 구축을 해볼 수 있었으며, 구글 Firebase 오픈소스를 사용해 푸시 알림을 받는 어플 자체를 구축해볼 수 있는 기회가 되었다. 

또한 프로젝트 진행의 전반적인 과정에 있어서, 객체 인식에 사용되는 Yolo 딥러닝 알고리즘을 통한 유해동물 탐지와 이를 사용자에게 알림을 주기 위한 어플과 웹서버의 구축, 아두이노 기기의 구동 방법 등을 알아보았는데 이를 수업에서 배운 IoT 시스템에 적용시켜 생각해보고 프로젝트를 진행할 수 있는 기회가 되었다고 생각한다. 

## 보완점

### 서버 문제
Yolo와 모바일 어플을 연결하는 웹서버 구축, 혹은 바로 모바일 어플에 결과물을 전송할 수 있는 체계를 구축하고자 한다.

또한 서버 구축이 가능하다면, 빛을 내서 동물을 쫓아내는 구동기를 아두이노를 사용해 구현해보고자 한다.

### 탐지의 강화
탐지 부분에 있어서, 멧돼지, 고라니, 고양이의 경우 어느정도 탐지가 되었으나 초기에 설정했던 유해동물들 중 꿩이나 뉴트리아는 제대로 탐지가 되지 않았다. 데이터 세트의 보완 및 학습의 추가를 통해 꿩과 뉴트리아의 탐지 정확도를 높이면 유해동물 감지 시스템이 더 효과적으로 구축될 수 있을 것으로 판단된다. 
